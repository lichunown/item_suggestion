...
def get_qid_results_cv(indri_rank_dict,ranker,test_qids):
    ...
    for qid in tqdm(test_qids):
        ...
        subtopic_str=qid_query_subtopic[qid]['subtopic'].replace("_"," ")
        #import pdb;pdb.set_trace()
        result_list=[]
        #import pdb;pdb.set_trace()
        if isinstance(ranker,FlagLLMReranker):
            scores=ranker.compute_score(rank_list,batch_size=2,prompt=prompt)
        else:
        ...
        for i in range(len(docids)):
            ...
            result_list.append([query,subtopic_str,doc_contents[i],score])
    dict_result=pyndeval.ndeval(qrels, run_list, measures=["alpha-nDCG@20"])
    return dict_result,result_list
if __name__=="__main__":
    ...
    for fold in fold_id:
        ...
        results_dict,result_list=get_qid_results_cv(indri_rank_dict,ranker,test_qids)
        eval_metrics=[results_dict[qid]['alpha-nDCG@20'] for qid in results_dict]
        eval_metrics_overall+=eval_metrics
        result_list_overall+=result_list
    ...
    import pickle,gzip
    with gzip.open("gemma-7b-teacher-data.pkl.gz",'wb') as f:
        pickle.dump(result_list_overall,f)
    import pdb;pdb.set_trace()
